...
def DAE(params_ae, method: str = 'layer-wise'):
    ...
    n_layer = config['AE_N_LAYER']
    unit = params_ae['ae_unit']
    ...
    ae_filename = f'AE_{dataset_name}_Unit-{unit}_{n_layer}-layer_A-{activation}_OA-{out_activation}_{loss_fn}' \
                  f'_E{epoch}_B{batch_size}_{opt}_{method} '
    ...
    if os.path.isfile(ae_path) and not config['AE_TRAINABLE']:
        ...
        encoded = tf.keras.layers.Dense(params_ae["ae_unit"], activation=params_ae['ae_activation'],
                                        kernel_initializer=tf.keras.initializers.GlorotNormal(seed=0),
                                        bias_initializer=tf.keras.initializers.Zeros())(input_img)
        decoded = tf.keras.layers.Dense(X_train.shape[1], activation=params_ae['ae_out_activation'],
                                        kernel_initializer=tf.keras.initializers.GlorotNormal(seed=0),
        ...
        for i in range(config['AE_N_LAYER'] - 1):
            ...
            new_layer = tf.keras.layers.Dense(params_ae["ae_unit"], activation=params_ae['ae_activation'],
                ...
                deep_autoencoder.layers[-2].output)
            decoded = tf.keras.layers.Dense(X_train.shape[1], activation=params_ae['ae_out_activation'],
                                            kernel_initializer=tf.keras.initializers.GlorotNormal(seed=0),
